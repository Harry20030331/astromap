...
function PointLabelText({
  ...
  const text =
    abbrev && PLANET_SHORT_LABEL[id] != null ? PLANET_SHORT_LABEL[id]! : (POINT_LABEL[id] ?? id);
  return (
    <span
      ...
      >
        <div className="grid shrink-0 grid-cols-[2.5rem_minmax(0,1fr)_2.5rem] items-center gap-x-1 border-b border-stone-200/90 bg-[var(--surface)] px-3 py-2.5 sm:px-4">
          ...
          <div className="min-w-0 px-1 text-center">
            ...
            {saving ? (
              <p className="mt-0.5 text-center text-xs text-stone-500">Saving…</p>
            ) : null}
          </div>
        ...
        <section>
          ...
          <div className="mt-2.5 space-y-3.5">
            {POINT_GROUPS.map((g) => {
              ...
              const isPlanets = g.title === "Planets";
              const ids = isAngles
              ...
              return (
                <div key={g.title}>
                  <p className="mb-1.5 text-sm font-semibold text-stone-700">{g.title}</p>
                  <div className={gridClass}>
                    {ids.map((id) => {
                      const on = included.has(id);
                      const disableOff = on && count <= 1;
                      const plainLabel = POINT_LABEL[id] ?? id;
                      return (
                        <button
                          key={id}
                          type="button"
                          disabled={disableOff}
                          onClick={() => toggle(id)}
                          title={disableOff ? "Keep at least one point" : plainLabel}
                          className={`flex items-center rounded-lg border text-left transition-colors ${
                            isPlanets
                              ? "gap-1 px-1.5 py-1.5 sm:gap-1.5 sm:px-2"
                              : "gap-2 px-2 py-1.5"
                          } ${
                            on
                        ...
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            <PointLabelText id={id} compact={isPlanets} abbrev={isPlanets} />
                          </span>
                        </button>
                      );
                    })}
        ...
        <section className="mt-5 border-t border-stone-100 pt-4">
          ...
          <p className="mt-1 text-xs text-stone-500">
            Type 0–15 (decimals allowed). Invalid values are flagged; valid entries save when you
            leave the field.
          </p>
            ...
            {ASPECT_ORB_KEYS.map((key) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex min-w-0 flex-col items-stretch sm:w-28">
                    <input
                      type="text"
                      inputMode="decimal"
                      autoComplete="off"
                      ...
                      className={`rounded-md border px-2 py-1.5 text-right text-sm tabular-nums outline-none focus-visible:ring-2 focus-visible:ring-sky-500/40 ${
                        err
